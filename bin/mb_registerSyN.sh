...
antsRegistration --dimensionality 3 --float 0 --collapse-output-transforms 1 --verbose \
  ...
  --transform Affine[0.1] --metric Mattes[$fixedfile,$movingfile,1,64,Random,0.30] --convergence [2000x2000x2000x2000,1e-6,10] --shrink-factors 6x4x2x1 --smoothing-sigmas 3x2x1x0vox --masks [$fixedmask,$movingmask] \
  --transform SyN[0.4,3,0] --metric CC[$fixedfile,$movingfile,1,4] --convergence [2000x1000x1000x100x100,1e-6,10] --shrink-factors 8x6x4x2x1 --smoothing-sigmas 4x3x2x1x0vox --masks [$fixedmask,$movingmask] \
  --transform SyN[0.2,3,0] --metric CC[$fixedfile,$movingfile,1,6] --convergence [20x20,1e-6,10] --shrink-factors 2x1 --smoothing-sigmas 1x0vox --masks [$fixedmask,$movingmask]
