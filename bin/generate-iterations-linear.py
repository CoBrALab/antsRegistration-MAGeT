...
while scale > 0.5:
    #print(str(min(int(8*resscale),max(1, int(scale * resscale)))))
    shrinks.append(str(min(int(8*resscale),max(1, int(scale * resscale)))))
...
for scale in reversed(range(int(shrinks[-1]))):
    ...
    blurs.append(
        str((scale / resscale) / (2.0 * math.sqrt(2.0 * math.log(2.0)))))
