...
fixedfile=$2
outputdir=$3
movingmask=$(echo $(dirname ${movingfile})/$(basename ${movingfile} | sed -r -e 's/_(t1|T1w|t2|T2w)\./_mask./g' || true) | grep mask || true)
fixedmask=$(echo $(dirname ${fixedfile})/$(basename ${fixedfile} | sed -r -e 's/_(t1|T1w|t2|T2w)\./_mask./g' || true) | grep mask || true)
...
if [[ -s ${fixedmask} && -s ${movingmask} ]]; then
  ...
  finalstage="--transform BSplineSyN[0.1,3,0,3] --metric CC[${fixedfile},${movingfile},1,6] --convergence [20,1e-6,10] --shrink-factors 1 --smoothing-sigmas 0 $finalmaskarg"
  ...
  initmaskarg=""
  finalmaskarg=""
  finalstage="--transform BSplineSyN[0.1,3,0,3] --metric CC[${fixedfile},${movingfile},1,4] --convergence [20,1e-6,10] --shrink-factors 1 --smoothing-sigmas 0"
fi
...
antsRegistration --dimensionality 3 --float 0 --collapse-output-transforms 1 ${MB_VERBOSE:-} --minc \
  ...
  --transform BSplineSyN[0.1,26,0,3] --metric CC[${fixedfile},${movingfile},1,4] --convergence [100x100x100x0x0,1e-6,10] \
