...
do
    ...
    do
        LabelOverlapMeasures 3 input/atlas/$(basename $label | sed 's#_t1.mnc##g') $label /tmp/$(basename $label).csv
        tail -n +2 /tmp/$(basename $label).csv | awk -vT="$(basename $label),$atlases,$templates,$fold," '{ print T $0 }' >> $output
    done
done
